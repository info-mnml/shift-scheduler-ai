        ...
        {shiftData.map((dayData, index) => (
          <motion.div 
            ...
            className={`p-3 border-2 rounded-lg min-h-[120px] transition-all duration-500 ${
              changedDates.has && changedDates.has(dayData.date) 
                ? 'border-orange-300 bg-orange-50 shadow-lg' 
                : 'border-gray-100 bg-white hover:border-blue-200'
            }`}
            animate={changedDates.has && changedDates.has(dayData.date) ? { scale: [1, 1.05, 1] } : {}}
            ...
            {dayData.shifts.map((shift, idx) => (
              ...
              >
                <div className="font-medium flex items-center">
                  ...
                  {showPreferred && shift.preferred && <CheckCircle className="h-3 w-3 ml-1 text-green-600" />}
                </div>
