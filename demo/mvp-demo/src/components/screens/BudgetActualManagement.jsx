...
import { Upload, CheckCircle, XCircle, FileText, DollarSign, Database, Download, TrendingUp, ArrowLeft, Users, Clock, AlertTriangle, ChevronLeft, ChevronRight } from 'lucide-react'
...
const BudgetActualManagement = () => {
  ...
  const parseCSV = (file, type) => {
    Papa.parse(file, {
      ...
      error: (error) => {
        ...
        const statusKey = type === 'workHours' ? 'workHours' : type === 'payroll' ? 'payroll' : type === 'salesActual' ? 'salesActual' : 'forecast'
  ...
  const handleMonthClick = async (monthData) => {
    if (!monthData.hasForecast || !monthData.hasWorkHours || !monthData.hasPayroll || !monthData.hasSalesActual) {
              ...
              {monthlyStatus.map((month) => (
                <div
                  ...
                  className={`border rounded-lg p-3 text-center transition-all ${
                    month.hasForecast && month.hasWorkHours && month.hasPayroll && month.hasSalesActual
